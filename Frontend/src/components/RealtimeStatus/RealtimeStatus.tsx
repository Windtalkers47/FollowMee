...
export default function RealtimeStatus() {
  ...
  return <Tooltip title={`${label} · ${updated}`}><Chip icon={icon} label={<><span>{label}</span><span className="realtime-updated"> · {updated}</span></>} color={snapshot.state === 'connected' ? 'success' : snapshot.state === 'offline' ? 'error' : 'warning'} variant="outlined" aria-label={`${label}. ${updated}`} sx={{ ml: 1, maxWidth: 250, '& .realtime-updated': { display: { xs: 'none', lg: 'inline' } }, '& .MuiChip-label': { overflow: 'hidden', textOverflow: 'ellipsis' } }} /></Tooltip>;
}
