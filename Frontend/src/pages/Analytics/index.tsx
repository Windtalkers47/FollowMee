...
type ProductFunnel = { funnel: Array<{ stage: string; count: number; conversionFromPrevious: number; dropOff: number }>; averageSecondsToStage: Record<string, number>; failures: Array<{ stage: string; reason: unknown }> };
...
export default function AnalyticsPage() {
  ...
  const funnelQuery = useQuery({ queryKey: ['analytics','product-funnel',startDate,endDate], queryFn: async () => (await axios.get(`${API_BASE_URL}/analytics/product-funnel`, { params: { from: startDate, to: endDate }, withCredentials: true })).data.data as ProductFunnel, enabled: tab === 2 && canOrganization && !rangeError });
  ...
  return <PageShell maxWidth={1400}>
    ...
    {tab === 3 ? <NotificationAnalytics startDate={startDate} endDate={endDate} /> : rangeError ? <Alert severity="warning">{rangeError}</Alert> : query.isError ? <PageError title={t('feature.analyticsLoadError')} message={t('feedback.networkHelp')} retryLabel={t('feedback.retry')} onRetry={() => void query.refetch()} /> : query.isLoading ? <PageLoading rows={2} label={t('feedback.loadingPage')} /> : <>
      ...
      {activeName === 'profiles' && canOrganization && <Card variant="outlined" sx={{ mt: 3, borderRadius: 3 }}><CardContent><Typography variant="h6" fontWeight={850}>{t('uat.funnel.title')}</Typography><Typography color="text.secondary" mb={2}>{t('uat.funnel.subtitle')}</Typography>{funnelQuery.isLoading ? <PageLoading rows={1} label={t('uat.funnel.loading')} /> : funnelQuery.isError ? <Alert severity="warning">{t('uat.funnel.error')}</Alert> : <Stack gap={1}>{funnelQuery.data?.funnel.map(stage => <Stack key={stage.stage} direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" sx={{ p: 1.5, borderRadius: 2, bgcolor: 'action.hover' }}><Typography fontWeight={750}>{stage.stage}</Typography><Typography>{t('uat.funnel.metric', { count: stage.count, conversion: (stage.conversionFromPrevious * 100).toFixed(1), drop: stage.dropOff })}{funnelQuery.data?.averageSecondsToStage[stage.stage] != null ? ` · ${funnelQuery.data.averageSecondsToStage[stage.stage]}s` : ''}</Typography></Stack>)}</Stack>}</CardContent></Card>}
    </>}
  </PageShell>;
}
