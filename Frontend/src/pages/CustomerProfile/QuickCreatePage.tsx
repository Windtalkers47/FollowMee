...
export default function QuickCreatePage() {
  ...
  const [form, setForm] = useState({ customerId: params.get('customerId') || '', firstName: '', lastName: '', email: '', phone: '', displayName: '', headline: '', primaryCtaLabel: '', primaryCtaUrl: '', templateKey: 'soft-mint' });
  ...
  const preview = useMemo<PublicProfileLanding>(() => ({ profileId: 'preview', slug: 'preview', displayName: form.displayName || form.firstName || t('profile.quick.previewName'), headline: form.headline || null, bio: null, avatarUrl: null, templateKey: form.templateKey, themeConfig: null, primaryCtaLabel: form.primaryCtaLabel || null, primaryCtaUrl: form.primaryCtaUrl || null, secondaryCtaLabel: null, secondaryCtaUrl: null, email: null, phone: null, address: null, links: [], seoTitle: form.displayName || form.firstName || t('profile.quick.previewName'), seoDescription: form.headline || null, publishedAt: null }), [form, t]);
  const submit = async () => { setSaving(true); setError(''); try { const profile = await publicProfileApi.quickCreate({ ...(mode === 'existing' ? { customerId: form.customerId } : {}), ...(mode === 'new' ? { customer: { firstName: form.firstName, lastName: form.lastName, email: form.email, phone: form.phone } } : {}), displayName: form.displayName || undefined, headline: form.headline, primaryCtaLabel: form.primaryCtaLabel, primaryCtaUrl: form.primaryCtaUrl, templateKey: form.templateKey }); navigate(`/customer-profile/${profile.profileId}/edit`); } catch (e) { setError(e instanceof Error ? e.message : t('profile.createError')); } finally { setSaving(false); } };
